...
gamma <- 0.009 ### stopnja natalitete muh, vstavi se načeloma kot vektor, lahko je tudi funkcija odvisna od temerature
...
#####################################################
okuzba<- geocode("Novo Mesto, Slovenija") ### lokacija, na kateri nastane okužba

########### Paramerer-stevilo okuzenih ##############
stevilo_okuzenih <- 50 ### stevilo okuzenih na dani lokaciji
####################################################
...
for(i in 1: dim[1]){
  for(j in 1:dim[2]){
    if(gradient_z[i,j] <0){
      gradient_z[i,j] <- (-1)* 1/(gradient_z[i,j])
    }
    if(gradient_o[i,j] <0){
      gradient_o[i,j] <- (-1)* 1/(gradient_o[i,j])
    }
    if(divergenca[i,j] <0){
      divergenca[i,j] <- (-1)* 1/(divergenca[i,j])
    }
...
#### del intenzivnosti okužbe, oznaka kot v članku (temp v celzijih) 
koeficienti <- function(Temperatura){
  a = 0.0002*Temperatura*(Temperatura-3.7)*(41.9 - Temperatura)^(1/2.7)
  return(a)
}
...
##############################################################
############ Parametri ###########################
c_2 = 0.95 # verjetnost prenosa z okuženega goveda na zdravo muho 
c_3 = 10^(-6) ### gradient zdravih muh
c_4 = 10^(-6)#.001#0.001 ### divergenca
c_5 = 0#.05#0.1 ### laplace
c_6 = 10^(-3) ### gradient okuzenih muh

leto = 8 ### opazovalni cas okuzbe
polmer <- 10
...
for(i in 1:leto){
  ...
  ostanki <- which(okuzeni!=0)%%dim(okuzeni)[1]
  stolpci <- which(okuzeni!=0)%/% dim(okuzeni)[1]
  stolpci[which(ostanki != 0)] <- stolpci[which(ostanki != 0)] +1
  vrstice <- ostanki
  vrstice[which(ostanki==0)] <- dim(okuzeni)[1]
  
  x_koordinata_o <- (-polmer:polmer)*(i) + vrstice
  y_koordinata_o<- (-polmer:polmer)*(i) + stolpci
  muhe_o[x_koordinata_o, y_koordinata_o] <- okuzeni[vrstice, stolpci]*st_muh
  
  
  
  muhe_z <- muhe_z + gamma * muhe_z -  c_2 *st_muh * okuzeni + c_3 * gradient_z + c_4*divergenca*muhe_z - c_5 * LAPLACE(muhe_z)
  muhe_o <- muhe_o  + c_2 * st_muh* okuzeni + c_6* gradient_o +c_4*divergenca*muhe_o- c_5 * LAPLACE(muhe_o)
...
points(mapa_x, mapa_y, pch=19, col="red", cex = 0.5)
